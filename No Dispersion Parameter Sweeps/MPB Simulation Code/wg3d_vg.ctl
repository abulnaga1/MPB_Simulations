...
(define-param eps_wgd 12)       ;Define the waveguide material
...
(set! geometry
    (list	(make block
			...
			(size infinity Y (/ Z 3))       
			(material (make dielectric (epsilon eps_sub))))    	;Diamond dielectric
		(make block
			(center 0 0 0)
			(size infinity w h)                         		;Block stretching infinitely far in x
			(material (make dielectric (epsilon eps_wgd))))))   	;Define waveguide to be made from GaAs
...
(find-k NO-PARITY omega 1 1 (vector3 1 0 0) 0.0001 kguess kmin kmax display-group-velocities)
