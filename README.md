...
![alt text](https://github.com/abulnaga1/MPB_Simulations/blob/master/Material%20Dispersion%20Single%20Parameter/vg_comparison.png "vg comparison") 

We can see that the built in MPB group velocity function is inaccurate as compared to the commercial software, whereas the finite difference method matches quite well. The latter simulation is run as follows: we use the [tuple_generator_dispersion.m](https://github.com/abulnaga1/MPB_Simulations/blob/master/Material%20Dispersion%20Single%20Parameter/MPB%20Simulation%20Code/tuple_generator_dispersion.m) to create an array of tuples containing the dielectric constants of our waveguide and substrate material as a function of frequency. The tuple generator makes use of analytical Sellmeier models from the literature. We then copy our list of tuples into the [batch submitter](https://github.com/abulnaga1/MPB_Simulations/edit/master/Material%20Dispersion%20Single%20Parameter/MPB%20Simulation%20Code/batch_submitter_dispersion.cmd) file. We then define our desired waveguide height and width in the same file and execute the job. The batch submitter runs a single simulation, defined in [wg3d_vg.ctl](https://github.com/abulnaga1/MPB_Simulations/tree/master/Material%20Dispersion%20Single%20Parameter/MPB%20Simulation%20Code) for each frequency as specified by the list of tuples, then saves the output k-point in a .dat file. We then use the code provided in the [analysis](https://github.com/abulnaga1/MPB_Simulations/tree/master/Material%20Dispersion%20Single%20Parameter/MPB%20Analysis%20Code) folder to process the data from the .dat files, and compute the effective index at each k-point. To compute the group velocity and dispersion as a function of wavelength as shown in the above figure, we can simply make use of matlab's built in numerical differentiation techniques. It is clearly not ideal to run so many simulations for a single device instance, but this is not what MPB was designed to do evidently. It is quite convenient if one is able to run simulations in parallel using a computation cluster or other method, as each data-point takes on the order of ~2-3 minutes. Speedups can also be achieved by reducing the simulation resolution or reducing the simulation window size. In both cases care should be taken however to make sure boundary effects do not lead to simulation artefacts.

I hope you found this tutorial useful; if you have any questions please feel free to contact me on github or by e-mail: alex dot abulnaga at princeton dot edu.
Happy simulating!
